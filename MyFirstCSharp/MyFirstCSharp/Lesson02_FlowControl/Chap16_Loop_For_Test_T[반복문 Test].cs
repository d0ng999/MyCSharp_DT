...
namespace MyFirstCSharp
{
    public partial class Chap16_Loop_For_Test_T : Form
    {
        ...
        {
            // 입력 시작 값부터 입력 종료 값까지의 짝수의 합과 홀수의 합을 구하기
            int iOdd = 0; // 홀수 값을 모두 더해 누적될 변수
            int iEven = 0; // 짝수 값을 모두 더해 누적될 변수

            int iStartValue = 0; // 입력 시작값 변수
            int iEndValue = 0;   // 입력 종료값 변수
            ...
            {
                sErrorMessage = "시작 입력 값이 종료 입력 값보다 크거나 같을 수 없습니다.";
                //MessageBox.Show("시작 입력 값이 종료 입력 값보다 크거나 같을 수 없습니다.");
                //return;
            }

            if (sErrorMessage != "")
            {
                // 벨리데이션 체크 시 예외 상황이 발생되었을 경우
                MessageBox.Show(sErrorMessage);
                return;
            }

            // 짝수와 홀수의 합, 과정 산출하기 로직
